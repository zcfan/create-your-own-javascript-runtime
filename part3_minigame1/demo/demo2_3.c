...
static const char* fragment_shader_text_head =
"#version 110\n"
"uniform float iTime;\n" // fragment shader 中用到 iTime 参数
"uniform vec2 iResolution;\n"; // fragment shader 中用到 iResolution 参数

static const char* fragment_shader_text_footer =
"void main()\n"
"{\n"
...
void runShadertoy(char* body)
{
    GLFWwindow* window;
    GLuint vertex_buffer, fragment_shader, program;
    GLint vpos_location, itime_location, iresolution_location;

    char *source = (char*) malloc((
        strlen(fragment_shader_text_head) + 
        strlen(body) + 
        strlen(fragment_shader_text_footer)) *
        sizeof(char)
    );
    ...
    // 拼接出完整的 fragment shader 代码
    strcat(source, fragment_shader_text_head);
    // strcat(source, body);
    strcat(source, body);
    strcat(source, fragment_shader_text_footer);
